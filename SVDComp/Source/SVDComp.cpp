...
SVDComp::SVDComp(){SetDrive(1.0f);}
SVDComp::~SVDComp(){}

void SVDComp::SetDrive(float drive)
{
    m_drive=drive;
    gain=1/exp(drive*0.3);
}

void SVDComp::Average(float* input, float* level, float dec)
{
    *level = dec*(*level) + (1-dec)*(*input);
}

void SVDComp::SoftClip(float* input, float* output)
{
    *output = tanh(*input);
}

void SVDComp::AudioSVD(AudioSampleBuffer& buffer, int channel, int offset)
{
...
void SVDComp::ClockProcess(float* LeftSample, float* RightSample)
{
